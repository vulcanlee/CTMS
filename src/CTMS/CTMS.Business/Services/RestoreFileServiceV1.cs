...
public class RestoreFileServiceV1
{
    ...
    public RestoreFileServiceV1(BackendDBContext backendDBContext,
    ...
    async Task PreparePersonFilesAsync(List<string> ProcessingMessing, Action RefreshMessages)
    {
        ...
        foreach (var currentRestorePath in directoryList)
        {
            ...
            #region 取得原有的紀錄資訊
            string DbDataFilename = Path.Combine(pathCurrent, pathDecompress, folder, "DbData.json");
            ...
            #region 處理每個檔案
            var files = Directory.GetFiles(currentRestorePath).ToList();
            athleteRecord.FileLists.Clear();

            CopyFile(fileListNode, athleteRecord, files, "_bar.png", FileTypeEnum.Bar);
            CopyFile(fileListNode, athleteRecord, files, "_hist.png", FileTypeEnum.Hist);
            CopyFile(fileListNode, athleteRecord, files, "_imat.png", FileTypeEnum.Imat);
            CopyFile(fileListNode, athleteRecord, files, "_lama.png", FileTypeEnum.Lama);
            CopyFile(fileListNode, athleteRecord, files, "_muscle5.png", FileTypeEnum.Muscle5);
            CopyFile(fileListNode, athleteRecord, files, "_muscleFat.png", FileTypeEnum.MuscleFat);
            CopyFile(fileListNode, athleteRecord, files, "_muscleQuality.png", FileTypeEnum.MuscleQuality);
            CopyFile(fileListNode, athleteRecord, files, "_myosteotosis.png", FileTypeEnum.Myosteotosis);
            CopyFile(fileListNode, athleteRecord, files, "_nama.png", FileTypeEnum.Nama);
            CopyFile(fileListNode, athleteRecord, files, "_sma.png", FileTypeEnum.Sma);
            CopyFile(fileListNode, athleteRecord, files, "_photo_", FileTypeEnum.Photo);
            CopyFile(fileListNode, athleteRecord, files, ".xlsx", FileTypeEnum.Xlsx);
            CopyFile(fileListNode, athleteRecord, files, ".png", FileTypeEnum.OnlyPng);
            #endregion
