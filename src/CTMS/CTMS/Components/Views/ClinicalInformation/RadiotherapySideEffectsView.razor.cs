...
public partial class RadiotherapySideEffectsView
{
    [Parameter]
    ...
    Survey放療副作用Node data = new();
    Survey放療副作用 header = new();
    ...
    {
        header = patientData.臨床資料.Survey放療副作用;
        if (isFirst)
        {
            sourceObjectJson = JsonConvert.SerializeObject(patientData.臨床資料.Survey放療副作用);
    ...
    async Task OnSaveAsync()
    {
        ...
        // data = patientData.臨床資料.問卷.放療副作用;
        patientAdapterModel.JsonData = patientData.ToJson();
        await PatientService.UpdateAsync(patientAdapterModel);

        targetObjectJson = JsonConvert.SerializeObject(patientData.臨床資料.Survey放療副作用);

        #region 更新操作日誌
        MyUserAdapterModel myUserAdapterModel = await AuthenticationStateHelper
        .GetUserInformation(authStateProvider);

        await OperationHistoryTraceService.AddAsync(OperationHistoryTraceAdapterModel.Build(myUserAdapterModel.Name, patientData.臨床資訊.SubjectNo, MagicObjectHelper.OperationCategory問卷_放療副作用, "-", "-"), sourceObjectJson, targetObjectJson, MagicObjectHelper.OperationCategory問卷_放療副作用);
        #endregion
