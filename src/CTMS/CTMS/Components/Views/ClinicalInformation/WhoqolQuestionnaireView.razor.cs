...
public partial class WhoqolQuestionnaireView
{
    [Parameter]
    ...
    SurveyWhooqol問卷Node data = new();
    SurveyWhooqol問卷 header = new();
    ...
    {
        header = patientData.臨床資料.SurveyWhooqol問卷;
        if (isFirst)
        {
            sourceObjectJson = JsonConvert.SerializeObject(patientData.臨床資料.SurveyWhooqol問卷);
    ...
    async Task OnSaveAsync()
    {
        ...
        // data = patientData.臨床資料.問卷.whooqol問卷;
        patientAdapterModel.JsonData = patientData.ToJson();
        await PatientService.UpdateAsync(patientAdapterModel);


        targetObjectJson = JsonConvert.SerializeObject(patientData.臨床資料.SurveyWhooqol問卷);

        #region 更新操作日誌
        MyUserAdapterModel myUserAdapterModel = await AuthenticationStateHelper
        .GetUserInformation(authStateProvider);

        await OperationHistoryTraceService.AddAsync(OperationHistoryTraceAdapterModel.Build(myUserAdapterModel.Name, patientData.臨床資訊.SubjectNo, MagicObjectHelper.OperationCategory問卷_Whooqol, "-", "-"), sourceObjectJson, targetObjectJson, MagicObjectHelper.OperationCategory問卷_Whooqol);
        #endregion
