...
public partial class FamilyHistorySurveyView
{
    [Parameter]
    ...
    async Task InitDataAsync(bool isFirst = true)
    {
        ...
        if (isFirst)
        {
            sourceObjectJson = JsonConvert.SerializeObject(patientData.臨床資料.Survey家族史問卷);
    ...
    async Task OnSaveAsync()
    {
        ...
        targetObjectJson = JsonConvert.SerializeObject(patientData.臨床資料.Survey家族史問卷);

        #region 更新操作日誌
        MyUserAdapterModel myUserAdapterModel = await AuthenticationStateHelper
        .GetUserInformation(authStateProvider);

        await OperationHistoryTraceService.AddAsync(OperationHistoryTraceAdapterModel.Build(myUserAdapterModel.Name, patientData.臨床資訊.SubjectNo, MagicObjectHelper.OperationCategory問卷_家族史問卷, "-", "-"), sourceObjectJson, targetObjectJson, MagicObjectHelper.OperationCategory問卷_家族史問卷);
        #endregion
