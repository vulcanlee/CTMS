...
public partial class QualityOfLifeSurveyView
{
    [Parameter]
    ...
    Survey生活品質問卷Node data = new();
    Survey生活品質問卷 header = new();
    ...
    {
        header = patientData.臨床資料.Survey生活品質問卷;
        if (isFirst)
        {
            sourceObjectJson = JsonConvert.SerializeObject(patientData.臨床資料.Survey生活品質問卷);
    ...
    async Task OnSaveAsync()
    {
        ...
        targetObjectJson = JsonConvert.SerializeObject(patientData.臨床資料.Survey生活品質問卷);

        #region 更新操作日誌
        MyUserAdapterModel myUserAdapterModel = await AuthenticationStateHelper
        .GetUserInformation(authStateProvider);

        await OperationHistoryTraceService.AddAsync(OperationHistoryTraceAdapterModel.Build(myUserAdapterModel.Name, patientData.臨床資訊.SubjectNo, MagicObjectHelper.OperationCategory問卷_生活品質問卷, "-", "-"), sourceObjectJson, targetObjectJson, MagicObjectHelper.OperationCategory問卷_生活品質問卷);
        #endregion
