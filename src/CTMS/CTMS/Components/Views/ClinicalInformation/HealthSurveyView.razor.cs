...
public partial class HealthSurveyView
{
    [Parameter]
    ...
    Survey健康問卷Node data = new();
    Survey健康問卷 header = new();
    ...
    {
        header = patientData.臨床資料.Survey健康問卷;
        if (isFirst)
        {
            sourceObjectJson = JsonConvert.SerializeObject(patientData.臨床資料.Survey健康問卷);
    ...
    async Task OnSaveAsync()
    {
        ...
        targetObjectJson = JsonConvert.SerializeObject(patientData.臨床資料.Survey健康問卷);

        #region 更新操作日誌
        MyUserAdapterModel myUserAdapterModel = await AuthenticationStateHelper
        .GetUserInformation(authStateProvider);

        await OperationHistoryTraceService.AddAsync(OperationHistoryTraceAdapterModel.Build(myUserAdapterModel.Name, patientData.臨床資訊.SubjectNo, MagicObjectHelper.OperationCategory問卷_健康, "-", "-"), sourceObjectJson, targetObjectJson, MagicObjectHelper.OperationCategory問卷_健康);
        #endregion
